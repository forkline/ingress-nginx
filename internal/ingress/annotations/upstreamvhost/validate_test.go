...
func TestValidate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			riskLevel: "high",
			anns:      map[string]string{"nginx.ingress.kubernetes.io/upstream-vhost": "example.com"},
			wantErr:   false,
		},
		{
			name:      "low risk annotation passes with low risk level",
			riskLevel: "low",
			anns:      map[string]string{"nginx.ingress.kubernetes.io/upstream-vhost": "example.com"},
			wantErr:   false,
		},
...
func TestGetDocumentation(t *testing.T) {
	...
	assert.Contains(t, fields, "upstream-vhost")
}
