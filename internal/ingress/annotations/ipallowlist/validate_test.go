...
func TestValidate(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "valid annotation passes with medium risk",
			riskLevel: "medium",
			anns:      map[string]string{"nginx.ingress.kubernetes.io/whitelist-source-range": "10.0.0.0/8"},
			wantErr:   false,
		},
		{
			name:      "medium risk annotation fails with low risk",
			riskLevel: "low",
			anns:      map[string]string{"nginx.ingress.kubernetes.io/whitelist-source-range": "10.0.0.0/8"},
			wantErr:   true,
		},
