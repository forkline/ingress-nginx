...
func TestValidate(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "valid auth-url passes with high risk",
			riskLevel: "high",
			anns:      map[string]string{"nginx.ingress.kubernetes.io/auth-url": "http://auth"},
			wantErr:   false,
		},
		{
			name:      "auth-url fails with low risk",
			riskLevel: "low",
			anns:      map[string]string{"nginx.ingress.kubernetes.io/auth-url": "http://auth"},
			wantErr:   true,
		},
...
func TestGetDocumentation(t *testing.T) {
	...
	assert.Contains(t, fields, "auth-url")
}
